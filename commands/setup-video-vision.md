---
description: "Interactive setup wizard for codex-video-vision — configure backend, whisper, frames, and verify dependencies"
---

# Setup Video Vision

Guide the user through configuring codex-video-vision step by step. Ask one question at a time using multiple choice. After each answer, proceed to the next step.
...
>
> **c) OpenAI Whisper API** — Good quality. Requires OPENAI_API_KEY. Paid per usage. The key can live in your shell environment, a local `.env`, or `~/.codex-video-vision/.env`.
>
> All backends use ffmpeg to extract video frames — Codex sees the frames directly.
...
### Frame Mode
> How should Codex receive the frames?
>
> **a) Images** (default) — Codex sees the actual frames (better perception, more tokens)
>
> **b) Descriptions** — Return timestamped frame references for a compact text-first pass (fewer tokens, less visual nuance)

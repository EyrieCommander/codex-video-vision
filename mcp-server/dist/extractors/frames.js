...
export async function extractFrames(videoPath, options) {
    ...
    args.push("-vf", `fps=${fps},scale=${resolution}:-1`, "-frames:v", String(maxFrames), "-q:v", "5", join(outputDir, "frame_%04d.jpg"));
 ...
 */
export function generateTimestampsForSegment(segment) {
 ...
 */
export async function extractFramesBySegments(videoPath, segments, baseOutputDir) {
    const allFrames = [];

 ...
 */
export function buildAnalysisCommand(videoPath, filters, workDir) {
...
// Parser functions
// ---------------------------------------------------------------------------
export function parseScdetOutput(stderr) {
    const results = [];
...
export function parseScdetFromMetaFile(content, threshold = 2) {
    const results = [];
...
export function parseBlackdetectOutput(stderr) {
    const results = [];
...
export function parseSilenceOutput(stderr) {
    const results = [];
    ...
    while ((m = endRe.exec(stderr)) !== null)
        ends.push({ t: parseFloat(m[1]), d: parseFloat(m[2]) });
...
export function parseFreezeOutput(stderr) {
    const results = [];
...
export function parseSitiOutput(stderr) {
    ...
    let m;
    while ((m = siRe.exec(stderr)) !== null)
        siValues.push(parseFloat(m[1]));
    while ((m = tiRe.exec(stderr)) !== null)
        tiValues.push(parseFloat(m[1]));
    const avg = (arr) => arr.length > 0 ? arr.reduce((a, b) => a + b, 0) / arr.length : undefined;
    return { siAvg: avg(siValues), tiAvg: avg(tiValues) };
}
export function parseBlurOutput(metaFileContent) {
    const results = [];
...
export function parseSignalstatsOutput(metaFileContent) {
    const results = [];
    const frameRe = /# frame:\d+.*?pts_time:([\d.]+)([\s\S]*?)(?=# frame:|$)/g;
...
// ---------------------------------------------------------------------------
export function deriveContentProfile(siAvg, tiAvg) {
    if (siAvg === undefined && tiAvg === undefined) {
        return "unknown (no motion analysis data)";
    }
    const siClass = siAvg === undefined ? "unknown" : siAvg > 50 ? "high" : siAvg > 25 ? "moderate" : "low";
    const tiClass = tiAvg === undefined ? "unknown" : tiAvg > 30 ? "high" : tiAvg > 10 ? "moderate" : "low";
    const descriptions = {
        high: {

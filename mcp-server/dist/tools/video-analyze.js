...
import { buildAnalysisCommand, parseScdetOutput, parseScdetFromMetaFile, parseBlackdetectOutput, parseSilenceOutput, parseFreezeOutput, parseSitiOutput, parseBlurOutput, parseSignalstatsOutput, parseEbur128Output, deriveContentProfile, } from "../extractors/analyzers.js";
...
export function registerVideoAnalyze(server) {
    server.tool("video_analyze", "Analyze video structure using ffmpeg filters. Returns scene changes, silence intervals, motion levels, and more. Use this before video_watch to plan which segments need detailed frame extraction. Does not extract frames.", {
        ...
        try {
            ...
            const ffmpegFilters = { ...filters, transcription: false };
            ...
            if (cmd !== null) {
                ...
                if (filters.blur && existsSync(cmd.videoMetaFile)) {
                    ...
                    for (const entry of blurData) {
                        const existing = analysis.frame_stats.find((fs) => fs.timestamp === entry.timestamp);
                        if (existing) {

 ...
 */
function buildViewablePool(extractedFrames, manifest) {
 ...
 */
function lookupTimestampsInManifest(manifest, timestamps) {
    const result = [];
...
export function registerVideoDetail(server) {
    server.tool("video_detail", "Drill into specific segments of a video. Extracts frames at variable FPS/resolution per segment. Separates extraction from viewing: use segments to extract, view/view_sample to control which frames are returned as images. When enable_index is on, frames are cached and deduplicated across calls.", {
        ...
        view: z
            ...
            .optional()
            .describe("Specific timestamps to return as images from the session cache"),
        view_sample: z
            .number()
            .min(1)
            .optional()
        ...
        skip_cached: z
            .boolean()
            .default(true)
            .describe("Skip re-writing of frames already cached at the same resolution"),
    }, async (params) => {
        // ------------------------------------------------------------------
        // Setup
        // ------------------------------------------------------------------
        ...
        if (params.segments && params.segments.length > 0) {
            ...
            try {
                extractedFrames = await extractFramesBySegments(safePath, params.segments, tmpWorkDir);
            }
            finally {

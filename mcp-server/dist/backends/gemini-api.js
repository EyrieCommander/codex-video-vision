...
export async function waitForFileActive(ai, file, options = {}) {
    if (!file.name) {
    ...
    while (current.state !== "ACTIVE") {
        if (current.state === "FAILED") {
            throw new Error(`Gemini file ${current.name} processing failed`);
        }
        if (Date.now() > deadline) {
            throw new Error(`Gemini file ${current.name} stuck in state ${current.state ?? "unknown"} after ${timeoutMs}ms`);
        }
...
export function parseGeminiAudioResponse(raw) {
    let parsed;
    try {
        parsed = JSON.parse(raw);
    }
    catch (err) {
        throw new Error(`Gemini returned non-JSON response despite structured output config: ${err instanceof Error ? err.message : String(err)}`);
    }
...
export async function analyzeWithGeminiApi(audioPath) {

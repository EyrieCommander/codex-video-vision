...
You have access to video understanding tools via the codex-video-vision MCP server.
...
**frame_mode:** Prefer `images` in Codex unless the user explicitly needs a compact text-only packet. `descriptions` returns the same timestamp set for later summarization, but it loses visual nuance.
